package org.snim2.checker.parser;

import org.snim2.checker.Checker;
import org.snim2.checker.ast.Formula;
import org.snim2.checker.error.SyntacticException;
...
`     */
	...
	if (org.snim2.checker.Checker.PARSE_DEBUG) {
	    System.out.println("\nParsing:");
	    System.out.println("-------");
	}
	absyn = parse();
     ...
     * @see    #reduce()
     */
    private static final Formula parse() 
	throws IOException,
	       SyntacticException {
      token = lexer.lex();
     ...
     * @see Val
     */
    private static final void shift() {
	if (org.snim2.checker.Checker.PARSE_DEBUG) {
	...
	if (token instanceof Val) {
	    switch(token.value()) {
	    case Token.TRUE : 
		val_stack.push(org.snim2.checker.ast.True.VALUE);
		break;
	    case Token.FALSE :
		val_stack.push(org.snim2.checker.ast.False.VALUE);
		break;
	    case Token.ATOM :
		val_stack.push(new org.snim2.checker.ast.Atom(((Atom)token).literal));
		break;
	    }
	} else {
     ...
     * @see #o_p_table
     */
    private static final void reduce() {
      if (org.snim2.checker.Checker.PARSE_DEBUG) {
	System.out.println("Reducing: "
      ...
      case Token.NEG : 
	val_stack.push(new org.snim2.checker.ast.Neg((Formula)val_stack.pop()));
	break;
      case Token.RPAREN :
      ...
      case Token.CONJ :
	{
	  ...
	  val_stack.push(new org.snim2.checker.ast.Conj((Formula)val_stack.pop(), 
					      temp));
	}
	break;
      case Token.DISJ : 
	{
	  Formula temp = (Formula)val_stack.pop();
	  val_stack.push(new org.snim2.checker.ast.Disj((Formula)val_stack.pop(),
					      temp));
	}
	break;
      case Token.IMPL : 
	{
	  Formula temp = (Formula)val_stack.pop();
	  val_stack.push(new org.snim2.checker.ast.Impl((Formula)val_stack.pop(), 
					      temp));
	}
	break;
      case Token.IFF :
	{
	  Formula temp = (Formula)val_stack.pop();
	  val_stack.push(new org.snim2.checker.ast.Iff((Formula)val_stack.pop(), 
					     temp));
	}
	break;
      }
